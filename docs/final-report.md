...
- iTunes Search API fallback: Spotify preview_url 없는 트랙의 미리듣기·앨범아트 보강
...
| 클라이언트 단위 테스트 | vitest | lib/, components/ 커버리지 게이트 |
...
- **Backend**: `pytest --cov=app --cov-fail-under=70` — 70% 미만이면 CI 실패
- **Client**: vitest 커버리지 70%+ 강제

### 5.3 ML 행동 테스트 (Behavioral Testing)

모델 정확도 외에 **의미적 검증**을 추가했다. 실제 한국어 긍정 발화("오늘 정말 기분이 좋고 행복해요")를 실서버에 전송하면 valence가 양수여야 한다는 극성(polarity) 제약을 자동으로 검증한다.

```python
def test_happy_voice_positive_valence():
    response = client.post("/predict", files={"audio": wav_bytes})
    assert response.json()["valence"] > 0
```
...
| NFR4.3 | 감정 분류 정확도 ≥ 70% | ✅ | AI Hub 한국어 데이터 fine-tuning |
...
- **NFR1.2 (동시 사용자 100명)** 부하 테스트를 실제로 측정하지 못했다. 학생 프로젝트 환경의 자원 제약과 기간 내 우선순위 조정 때문이었다.
