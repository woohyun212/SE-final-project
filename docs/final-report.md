...
27개 FR 의 최종 구현 기준 추적성. (✅ 구현 / ⚠️ 대체 구현 / ❌ 미구현 — 우선순위 M=Must, S=Should, C=Could)

| ID | 요구사항 | 우선 | 달성 | 비고 |
|---|---|---|---|---|
| FR1.1 | 회원가입 | M | ✅ | |
| FR1.2 | 로그인/로그아웃 | M | ✅ | 서버측 refresh token revoke 포함 |
| FR1.3 | 프로필 수정 | S | ❌ | 미구현 (우선순위 후순위) |
| FR1.4 | 계정 탈퇴 (영구 삭제) | M | ✅ | `DELETE /auth/me` + 연관 데이터 cascade 삭제 |
| FR2.1 | 음성 녹음 | M | ✅ | Electron MediaRecorder |
| FR2.2 | TLS 안전 전송 | M | ✅ | HTTPS + http→https 자동 승격 |
| FR2.3 | 녹음 시각 피드백 | S | ✅ | 타이머 + 파형형 애니메이션 |
| FR2.4 | 최대 녹음 시간 제한 | M | ✅ | 자동 종료 타이머 |
| FR3.1 | 음성→감정 벡터 | M | ✅ | wav2vec2 (ML 서버) |
| FR3.2 | 음성→텍스트 (STT) | M | ✅ | Whisper 어댑터 |
| FR3.3 | 텍스트→맥락 (LLM) | M | ✅ | Gemini ContextAnalyzer |
| FR3.4 | 감정+맥락 융합 | M | ✅ | EmotionFusion |
| FR4.1 | 유사도 매칭 | M | ✅ | 코사인 유사도 |
| FR4.2 | 상위 K=10곡 | M | ✅ | |
| FR4.3 | LLM 추천 이유 | M | ✅ | rule-based fallback 포함 |
| FR4.4 | 결과 캐싱 (TTL 1h) | C | ❌ | 선택 요구사항, 미구현 |
| FR5.1 | 2D 감정 차트 | M | ✅ | EmotionMusicChart |
| FR5.2 | 곡 리스트 (앨범아트) | M | ✅ | iTunes Search 폴백으로 앨범아트 보강 |
| FR5.3 | 추천 이유 표시 | M | ✅ | RecommendationReasonCard |
| FR5.4 | 곡 재생 (미리듣기) | M | ✅ | iTunes Search preview 폴백 |
| FR6.1 | 좋아요/싫어요 | M | ✅ | |
| FR6.2 | 재생 이벤트 로깅 | M | ✅ | start/end/complete |
| FR6.3 | 피드백 프로필 누적 | M | ✅ | running mean |
| FR6.4 | 누적 피드백 가중치 반영 | M | ✅ | 추천 점수 ±0.3 |
| FR6.5 | 추천 이력 조회 | S | ✅ | |
| FR7.1 | YouTube 시드 트랙 추출 | M | ⚠️ | Kaggle Spotify 데이터셋 적재로 대체 |
| FR7.2 | Spotify 매칭 적재 | M | ⚠️ | 사전 매칭된 데이터셋 적재로 대체 |
| FR7.3 | 주기적 카탈로그 새로고침 | S | ❌ | 수동 적재 스크립트만 |
| FR7.4 | 라이선스/지역 필터 | S | ❌ | 미구현 |

**요약**: Must(M) 17개 전부 ✅(FR7.1/7.2 는 데이터셋 적재로 대체) — 핵심 기능 완수. 미구현은 Should/Could 우선순위(FR1.3·FR4.4·FR7.3·FR7.4)에 한정된다.
...
| 응답 전송 + 렌더링 | ≤ 100ms | |
| **총합 (설계 예산)** | **≤ 3,000ms** | 설계 목표 |

> 위 표는 **설계 단계의 응답 시간 예산**이다. STT·ML 병렬화로 대부분 단계는 예산 내에 들지만, LLM 추천 이유 생성은 초기 구현에서 23초+ 지연이 관측되어(#177) 호출 트랙 수 제한으로 완화했다. 부하 환경에서의 **실측 P95 측정은 운영 기간 제약으로 수행하지 못했다** — 따라서 NFR1.1 은 "설계 예산 충족, 실측 미검증"(⚠️ 부분)으로 분류한다(§7).
...
|---|---|---|---|
| NFR1.1 | 추천 응답 P95 ≤ 3초 | ⚠️ 부분 | STT·ML 병렬화(asyncio.gather)로 설계 예산 충족(§3.5). 단 LLM 추천 이유 지연(#177)이 변수이며 **실측 P95 미수행** — 설계상 목표, 실측 미검증 |
| NFR1.2 | 동시 사용자 100명 | ⚠️ 미측정 | 부하 테스트(locust/k6) 미수행 — 운영 기간·자원 제약 |
| NFR1.3 | ML 추론 ≤ 1,500ms | ✅ | wav2vec2-base, GPU 서빙 |
| NFR2.1 | 백엔드 가용성 ≥ 99% | ⚠️ 미측정 | restart 정책·헬스체크 구성, 월간 가동률 모니터링은 미구축 |
...
| NFR3.2 | 음성 원본 즉시 폐기 | ✅ | `audio_bytes` 인메모리 처리, DB·디스크 미저장(요청 핸들러 종료 시 GC) |
| NFR3.3 | STT 텍스트 동의 기반 저장 | ✅ | transcript 영속화 안 함(미저장) — 동의 없는 저장 자체가 발생하지 않음 |
...
| NFR5.2 | 색맹 대응 팔레트 | ✅ | EmotionMusicChart Okabe-Ito 팔레트 + 형태(stroke) 보조 구분 |
...
| CI flaky 504 오류 | Electron 바이너리 다운로드 타임아웃 | 바이너리 다운로드 스킵 + npm 캐시 | c43996a |
...
| 2단계 테스트 보고서 | `docs/submissions/phase2/소프트웨어공학_2단계_테스트보고서.docx`, `docs/03-test-report.md` |
| NFR4.2 Precision@10 평가 결과 | `docs/submissions/phase3/nfr42-precision.md` |
